...
import { syncGoogleCalendarForOrg } from '@/server/integrations/google-calendar-sync';

// sync logic moved to shared utility
...
export async function POST(request) {
  try {
    ...
    const result = await syncGoogleCalendarForOrg({ organizationId, targetEmail })
...
export async function GET(request) {
  try {
    ...
    let sel = service
      .from('integrations')
      .select('updated_at, created_at, access_json, account_email')
      .eq('organization_id', organizationId)
      .eq('type', 'google_calendar');

    if (targetEmail) {
      sel = sel.eq('account_email', targetEmail).limit(1);
    } else {
      sel = sel.order('updated_at', { ascending: false }).limit(1);
    }

    const { data: integrationsData, error: selError } = await sel;
    if (selError) {
      return NextResponse.json({ error: 'Failed to get integration', details: selError.message }, { status: 500 });
    }

    const integration = Array.isArray(integrationsData) ? integrationsData[0] : integrationsData;
